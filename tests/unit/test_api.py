...
class TestSynchronousExecuteEndpoint:
    ...
    def test_execute_with_textentries_payload(
        self, mock_init_agent, client_without_mcp, sample_execution_payload
    ):
...
class TestSSEStreamingEndpoints:
    ...
    @patch("src.utils.mcp_utils.init_agent")
    def test_start_execution_creates_job(
        self, mock_init_agent, client_without_mcp, sample_execution_payload
    ):
    ...
    def test_start_execution_with_invalid_payload(self, client_without_mcp):
        ...
        response = client_without_mcp.post(
...
class TestAsyncSSEStreaming:
    ...
    @patch("src.utils.mcp_utils.init_agent")
    async def test_stream_execution_flow(
        self, mock_init_agent, async_client_without_mcp, sample_execution_payload
    ):
        ...
        # Start execution
        response = await async_client_without_mcp.post(
            "/api/v1/stream-execute", json=sample_execution_payload
        )
        assert response.status_code == 200
...
class TestCORSMiddleware:
    ...
    def test_preflight_request(self, client_without_mcp):
        ...
        response = client_without_mcp.options(
            "/api/v1/stream-execute",
            headers={
